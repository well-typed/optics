...
-- | @'each' :: 'Traversal' i (a, a, a, a, a, a, a, a, a) (b, b, b, b, b, b, b,
-- b, b) a b@
instance
  (a ~ a1, a ~ a2, a ~ a3, a ~ a4, a ~ a5, a ~ a6, a ~ a7, a ~ a8,
   b ~ b1, b ~ b2, b ~ b3, b ~ b4, b ~ b5, b ~ b6, b ~ b7, b ~ b8,
   i ~ Int
  ) => Each i (a, a1, a2, a3, a4, a5, a6, a7, a8)
...
instance i ~ Either () () => Each i (Complex a) (Complex b) a b where
  each = ixTraversalVL $ \f (a :+ b) -> (:+) <$> f (Left ()) a <*> f (Right ()) b
  {-# INLINE each #-}

-- | @'each' :: 'Traversal' k ('Map' k a) ('Map' k b) a b@
instance k ~ k' => Each k (Map k a) (Map k' b) a b

-- | @'each' :: 'Traversal' i ('Map' c a) ('Map' c b) a b@
instance i ~ Int => Each i (IntMap a) (IntMap b) a b

-- | @'each' :: 'Traversal' i [a] [b] a b@
instance i ~ Int => Each i [a] [b] a b

-- | @'each' :: 'IxTraversal' Int (NonEmpty a) (NonEmpty b) a b@
instance i ~ Int => Each i (NonEmpty a) (NonEmpty b) a b

-- | @'each' :: 'IxTraversal' () ('Identity' a) ('Identity' b) a b@
instance i ~ () => Each i (Identity a) (Identity b) a b

-- | @'each' :: 'IxTraversal' () ('Maybe' a) ('Maybe' b) a b@
instance i ~ () => Each i (Maybe a) (Maybe b) a b

-- | @'each' :: 'IxTraversal' Int ('Seq' a) ('Seq' b) a b@
instance i ~ Int => Each i (Seq a) (Seq b) a b

-- | @'each' :: 'IxTraversal' [Int] ('Tree' a) ('Tree' b) a b@
instance i ~ [Int] => Each i (Tree a) (Tree b) a b
