...
productOf :: (Is k A_Fold, Num a) => Optic' k s a -> s -> a
productOf o = foldlOf' o (*) 1
{-# INLINE productOf #-}

sumOf :: (Is k A_Fold, Num a) => Optic' k s a -> s -> a
sumOf o = foldlOf' o (+) 0
{-# INLINE sumOf #-}

asumOf :: (Is k A_Fold, Alternative f) => Optic' k s (f a) -> s -> f a
asumOf o = foldrOf o (<|>) empty
{-# INLINE asumOf #-}

msumOf :: (Is k A_Fold, MonadPlus m) => Optic' k s (m a) -> s -> m a
msumOf o = foldrOf o mplus mzero
{-# INLINE msumOf #-}

elemOf :: (Is k A_Fold, Eq a) => Optic' k s a -> a -> s -> Bool
elemOf o = anyOf o . (==)
{-# INLINE elemOf #-}

notElemOf :: (Is k A_Fold, Eq a) => Optic' k s a -> a -> s -> Bool
notElemOf o = allOf o . (/=)
{-# INLINE notElemOf #-}

concatOf :: Is k A_Fold => Optic' k s [a] -> s -> [a]
concatOf o = foldMapOf o id
{-# INLINE concatOf #-}

concatMapOf :: Is k A_Fold => Optic' k s a -> (a -> [b]) -> s -> [b]
concatMapOf = foldMapOf
{-# INLINE concatMapOf #-}

lengthOf :: Is k A_Fold => Optic' k s a -> s -> Int
